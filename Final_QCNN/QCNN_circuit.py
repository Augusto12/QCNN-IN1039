...
def conv_layer1(U, params):
    U_0 = unitary.Conv_Ansatze_Selector(U[0])
    U_1 = unitary.Conv_Ansatze_Selector(U[1])
    U_0(params[0], wires=[0, 7])
    for i in range(0, 8, 2):
        U_1(params[1], wires=[i, i + 1])
    for i in range(1, 7, 2):
        U_0(params[0], wires=[i, i + 1])
def conv_layer2(U, params):
    U_0 = unitary.Conv_Ansatze_Selector(U[0])
    U_1 = unitary.Conv_Ansatze_Selector(U[1])
    U_1(params[1], wires=[0, 6])
    U_0(params[0], wires=[0, 2])
    U_0(params[0], wires=[4, 6])
    U_1(params[1], wires=[2, 4])
def conv_layer3(U, params):
    U_0 = unitary.Conv_Ansatze_Selector(U[0])
    U_0(params, wires=[0,4])
...
def QCNN_structure(U, params, U_params):
    INDEX_SECOND_LAYER = U_params[0] * 3 + 2 * 3

    param1 = [params[0:U_params[0]], params[INDEX_SECOND_LAYER : INDEX_SECOND_LAYER + U_params[1]]]
    param2 = [params[U_params[0]: 2 * U_params[0]], params[INDEX_SECOND_LAYER + U_params[1]:]]
